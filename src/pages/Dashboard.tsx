...
import { Phone, Users, BookOpen, Star, ShieldCheck, Calendar } from 'lucide-react';
...
function AdminDashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const sdrMap = new Map(allSDRs.map((sdr: any) => [sdr.id, sdr.name]));

      const teamSpinScores = await baserowService.getTeamSpinScores(user.organizationId, dateRange.startDate, dateRange.endDate);
      
      // Simulação de dados para os widgets, pois a lógica de cálculo ainda não foi criada
      ...
      const analysesMap = new Map(analyses.map((a: any) => [a[baserowService.FIELD_IDS.analyses.callRecording]?.[0]?.id, a]));

      const combinedData: CombinedCallData[] = recordings
        .sort((a: any, b: any) => new Date(b[baserowService.FIELD_IDS.callRecordings.callDate]).getTime() - new Date(a[baserowService.FIELD_IDS.callRecordings.callDate]).getTime())
        .map((rec: any) => {
          const sdrId = rec[baserowService.FIELD_IDS.callRecordings.sdr]?.[0]?.id || null;
          const analysis = analysesMap.get(rec.id);
          return {
            call_id: rec.id.toString(),
            sdr_id: sdrId,
            prospect_name: rec[baserowService.FIELD_IDS.callRecordings.prospectName],
            call_date: rec[baserowService.FIELD_IDS.callRecordings.callDate],
            efficiency_score: analysis?.[baserowService.FIELD_IDS.analyses.efficiencyScore] || 0,
            status: 'Analisada', // Placeholder
            sdr_name: sdrId ? sdrMap.get(sdrId) : 'N/A',
            call_duration_seconds: rec[baserowService.FIELD_IDS.callRecordings.duration] || 0,
          };
        });

      setRecentCalls(combinedData.slice(0, 5));
      
      const totalCalls = combinedData.filter(call => {
      ...
      const avgEfficiencyScore = combinedData.length > 0 ? Math.round(totalScore / combinedData.length) : 0;
      ...
      setDashboardData({
        ...
        spinAnalysis: [
          { subject: 'Situação', A: teamSpinScores.situation, fullMark: 100 },
          { subject: 'Problema', A: teamSpinScores.problem, fullMark: 100 },
          { subject: 'Implicação', A: teamSpinScores.implication, fullMark: 100 },
          { subject: 'Necessidade', A: teamSpinScores.need_payoff, fullMark: 100 },
        ],
      });
    } catch (error) {
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 bg-surface p-6 rounded-2xl shadow-lg border border-gray-100 h-[400px]">
          ...
          {/* CORREÇÃO APLICADA AQUI: O KPIWidget é envolvido por uma div que ocupa 2 colunas no grid. */}
